...
import { RiCheckLine, RiExpandUpDownLine, RiCloseLine } from "@remixicon/react"
...
export function MultiSelectTrigger({
  ...
  return (
    <PopoverTrigger asChild>
      ...
      >
        {children}
        <RiExpandUpDownLine className="ml-auto size-4 shrink-0 opacity-50" />
      </Button>
    </PopoverTrigger>
  )
}
      ...
      {[...selectedValues]
        ...
        .map((value) => (
          ...
          >
            {items.get(value)}
            {clickToRemove && (
              <RiCloseLine className="size-2 text-muted-foreground group-hover:text-destructive" />
            )}
          </Badge>
        ))}
      <Badge
